# ui_2026_v2_glass.py – TOP 2026 preview V2: Glass / Premium. NEMĚŇ produkční ui.py.
# Glassmorphism cards, jemný gradient v headeru, soft shadows, glow na accent.
...
# V2 Glass: světlejší karty, bordery, glow
BG_APP = "#0B0F14"
BG_CARD = "#151B24"
BG_HEADER = "#0F172A"
HEADER_TOP = "#1a2332"
...
class PDFCheckUI_2026_V2:
    """Preview V2 – glassmorphism, gradient header, soft shadows, accent glow."""
    ...
    def __init__(self, root, on_check_callback, on_api_key_callback, api_url="",
        ...
        self.root.title("DokuCheck Agent – Preview V2 Glass (2026)")
    ...
    def _build_layout(self):
        ...
        # Sidebar – glass card
        sidebar = ctk.CTkFrame(self.root, fg_color=BG_HEADER, width=SIDEBAR_W, corner_radius=0, border_width=0)
        sidebar.grid(row=0, column=0, sticky="nswe")
        sidebar.grid_propagate(False)
        ctk.CTkLabel(sidebar, text="DokuCheck", font=(FONT_STACK[0], FS_20, "bold"), text_color=TEXT).pack(pady=(20, 0))
        ctk.CTkLabel(sidebar, text="Preview V2 Glass", font=(FONT_STACK[0], FS_12), text_color=TEXT_MUTED).pack(pady=(0, 16))
        ...
        ctk.CTkButton(sidebar, text="Otevřít Web", command=self._open_web, font=(FONT_STACK[0], FS_14), width=180, fg_color=ACCENT, hover_color=ACCENT_GLOW).pack(pady=8, padx=16, fill=tk.X)
        ...
        self.login_btn = ctk.CTkButton(sidebar, text="Přihlásit", command=self.show_api_key_dialog, font=(FONT_STACK[0], FS_14), width=180, fg_color=BORDER_GLASS)
        ...
        # Command bar – gradient-like: dva pruhy (světlejší nahoře)
        bar_top = ctk.CTkFrame(main, fg_color=HEADER_TOP, height=4, corner_radius=0)
        bar_top.grid(row=0, column=0, sticky="ew")
        bar_top.grid_propagate(False)
        bar = ctk.CTkFrame(main, fg_color=BG_CARD, height=48, corner_radius=0, border_width=1, border_color=BORDER_GLASS)
        bar.grid(row=1, column=0, sticky="ew")
        bar.grid_propagate(False)
        main.grid_rowconfigure(1, weight=0)
        ctk.CTkButton(bar, text="Přidat soubory", command=self.add_files, font=(FONT_STACK[0], FS_14), width=120, fg_color=ACCENT, hover_color=ACCENT_GLOW).pack(side=tk.LEFT, padx=8, pady=6)
        ctk.CTkButton(bar, text="+ Složka", command=self.add_folder, font=(FONT_STACK[0], FS_14), width=90, fg_color=ACCENT, hover_color=ACCENT_GLOW).pack(side=tk.LEFT, padx=4, pady=6)
        ctk.CTkButton(bar, text="Vymazat", command=self.clear_queue, font=(FONT_STACK[0], FS_14), width=80, fg_color=BORDER_GLASS).pack(side=tk.LEFT, padx=4, pady=6)
        self.check_btn = ctk.CTkButton(bar, text="ODESLAT KE KONTROLE", command=self.on_check_clicked, font=(FONT_STACK[0], FS_16, "bold"), fg_color=ACCENT, hover_color=ACCENT_GLOW, height=36, border_width=1, border_color=ACCENT_GLOW)
        ...
        # Queue – glass card s border a radius
        left = ctk.CTkFrame(content, fg_color=BG_CARD, corner_radius=CARD_RADIUS, border_width=1, border_color=BORDER_GLASS)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 6))
        left.grid_columnconfigure(0, weight=1)
        left.grid_rowconfigure(1, weight=1)
        ctk.CTkLabel(left, text="Fronta", font=(FONT_STACK[0], FS_16, "bold"), text_color=TEXT).grid(row=0, column=0, sticky="w", padx=14, pady=(14, 8))
        ...
        right = ctk.CTkFrame(content, fg_color=BG_CARD, corner_radius=CARD_RADIUS, border_width=1, border_color=BORDER_GLASS)
        right.grid(row=0, column=1, sticky="nsew", padx=(6, 0))
        right.grid_columnconfigure(0, weight=1)
        right.grid_rowconfigure(1, weight=1)
        ctk.CTkLabel(right, text="Detail výsledku", font=(FONT_STACK[0], FS_16, "bold"), text_color=TEXT).grid(row=0, column=0, sticky="w", padx=14, pady=(14, 8))
        self.detail_text = ctk.CTkTextbox(right, font=(FONT_STACK[0], FS_14), fg_color=BG_APP, text_color=TEXT, wrap="word", border_width=1, border_color=BORDER)
        ...
        self._progress_row.grid(row=3, column=0, sticky="ew", padx=10, pady=6)
        ...
        self.progress = ctk.CTkProgressBar(self._progress_row, height=10, progress_color=ACCENT)
    ...
    def _setup_dnd_overlay(self):
        if not TKINTERDND_AVAILABLE:
            return
        try:
            overlay = tk.Frame(self.root, bg="#0B0F14")
            overlay.place(relx=0, rely=0, relwidth=1, relheight=1)
            overlay.place_forget()
            back = tk.Frame(overlay, bg="#0B0F14")
            back.place(relx=0, rely=0, relwidth=1, relheight=1)
            card = tk.Frame(overlay, bg=BORDER_GLASS, highlightbackground=ACCENT, highlightthickness=2)
            card.place(relx=0.5, rely=0.5, anchor=tk.CENTER, width=320, height=140)
            tk.Label(card, text="↓", font=(FONT_STACK[0], 32), fg=ACCENT_GLOW, bg=BORDER_GLASS).pack(pady=(16, 4))
            tk.Label(card, text="Pustit soubory k analýze", font=(FONT_STACK[0], FS_20, "bold"), fg=TEXT, bg=BORDER_GLASS).pack(pady=(0, 8))
    ...
    def update_queue_display(self):
        ...
        for qidx, item in enumerate(self.queue_display):
            row = ctk.CTkFrame(self.queue_scroll, fg_color=BORDER_GLASS, corner_radius=8, height=46, border_width=1, border_color=BORDER)
            ...
            lbl.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4, pady=10)
            ...
            pill.pack(side=tk.RIGHT, padx=10, pady=10)
    ...
    def show_api_key_dialog(self):
        ...
        ctk.CTkButton(dialog, text="Vyzkoušet zdarma", command=do_trial, font=(FONT_STACK[0], FS_14), fg_color=ACCENT, hover_color=ACCENT_GLOW).pack(pady=8)
        ...
        ctk.CTkButton(dialog, text="Přihlásit se", command=do_login, font=(FONT_STACK[0], FS_14), fg_color=ACCENT, hover_color=ACCENT_GLOW).pack(pady=12)


def create_app_2026_v2(on_check_callback, on_api_key_callback, api_url="",
                       on_login_password_callback=None, on_logout_callback=None, on_get_max_files=None,
                       on_after_login_callback=None, on_after_logout_callback=None, on_get_web_login_url=None,
                       on_send_batch_callback=None):
    """Vytvoří a vrátí (root, app) pro preview V2 Glass – stejný podpis jako create_app."""
