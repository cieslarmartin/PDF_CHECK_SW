...
#
# Spuštění: python pdf_dokucheck_pro_v41_with_api.py
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .batch-content { display: none; padding-left: 16px; }
        .batch-content.visible { display: block; }

        /* Folder */
        .folder { border-left: 1px dashed #e5e7eb; margin-left: 8px; }
        .folder-header { display: flex; align-items: center; gap: 8px; padding: 6px 12px; cursor: pointer; }
        .folder-header:hover { background: #f9fafb; }
        .folder-arrow { color: #9ca3af; font-size: 0.7em; transition: transform 0.2s; }
        .folder-arrow.collapsed { transform: rotate(-90deg); }
        .folder-icon { color: #fbbf24; }
        .folder-name { font-weight: 500; color: #374151; font-size: 0.8em; }
        .folder-stats { font-size: 0.7em; color: #9ca3af; }
        .folder-content { display: none; margin-left: 24px; border-left: 1px dashed #e5e7eb; padding-left: 8px; }
        ...
        <div id="layout">
            <div id="sidebar">
                <div class="sidebar-content">
                    ...
                    <div class="filter-section" style="margin-top:10px;padding-top:0;border-top:none;">
                        <div class="filter-title">Filtr Razítko (TSA)</div>
                        <div class="filter-buttons" id="filter-tsa">
                            <button class="filter-btn active" data-value="all">Vše</button>
                            <button class="filter-btn" data-value="tsa">✓ TSA</button>
                            <button class="filter-btn" data-value="local">Lokální</button>
                        </div>
                    </div>
                    ...
                    <div class="legend">
                        ...
                        <div class="legend-grid">
                            <div class="legend-item"><span class="badge badge-green">A-3</span> OK</div>
                            <div class="legend-item"><span class="badge badge-yellow">A-2</span> Starší</div>
                            <div class="legend-item"><span class="badge badge-green">TSA</span> Kvalif.</div>
                            <div class="legend-item"><span class="badge badge-yellow">LOK</span> Lokální</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-header">
                    ...
                    <div class="table-header-cell">
                        ...
                        <div class="filter-dropdown" id="dropdown-pdfa">
                            ...
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('pdfa','A3')"><span class="filter-dot" style="background:#22c55e"></span>PDF/A-3 ✓</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('pdfa','A2')"><span class="filter-dot" style="background:#eab308"></span>PDF/A-2</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('pdfa','A1')"><span class="filter-dot" style="background:#f97316"></span>PDF/A-1</button>
                    ...
                    <div class="table-header-cell">
                        ...
                        <div class="filter-dropdown" id="dropdown-sig">
                            <button class="filter-dropdown-item clear" onclick="setHeaderFilter(null,null)">✕ Zobrazit vše</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('sig','OK')"><span class="filter-dot" style="background:#22c55e"></span>Kompletní</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('sig','PARTIAL')"><span class="filter-dot" style="background:#eab308"></span>Částečný</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('sig','FAIL')"><span class="filter-dot" style="background:#ef4444"></span>Chybí</button>
                        </div>
                    </div>
                    <div class="table-header-cell">
                    ...
                    <div class="table-header-cell">
                        ...
                        <div class="filter-dropdown" id="dropdown-tsa" style="right:0;left:auto;">
                            <button class="filter-dropdown-item clear" onclick="setHeaderFilter(null,null)">✕ Zobrazit vše</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('tsa','TSA')"><span class="filter-dot" style="background:#22c55e"></span>TSA (kvalif.)</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('tsa','LOCAL')"><span class="filter-dot" style="background:#eab308"></span>Lokální</button>
                            <button class="filter-dropdown-item" onclick="setHeaderFilter('tsa','NONE')"><span class="filter-dot" style="background:#9ca3af"></span>Žádné</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-content">
                <div id="tab-about">
                    ...
                    <ul>
                        ...
                        <li>✓ Časové razítko (TSA vs lokální)</li>
                    </ul>
        ...
        <div class="modal" style="max-width:600px;">
            ...
            <div class="modal-content" style="padding:24px;font-size:0.9em;line-height:1.7;">
                ...
                <p style="margin:0 0 16px 0;color:#4b5563;">
                    ...
                    • <strong>Časové razítko</strong> – TSA (kvalifikované) vs. lokální
                </p>
...
function expandLevel(level) {
    ...
    document.querySelectorAll('.folder').forEach(folder => {
        const depth = (folder.style.marginLeft ? parseInt(folder.style.marginLeft) / 16 : 0);
...
function getPdfaBadge(f) {
    if (f.pdfaStatus === 'OK') return '<span class="badge badge-green">A-3</span>';
    if (f.pdfaVersion === 2) return '<span class="badge badge-yellow">A-2</span>';
    if (f.pdfaVersion === 1) return '<span class="badge badge-orange">A-1</span>';
    return '<span class="badge badge-red">NE</span>';
}
function getSigBadge(f) {
    if (f.sig === 'OK') return '<span class="badge badge-green">ANO</span>';
    if (f.sig === 'PARTIAL') return '<span class="badge badge-yellow">ČÁST.</span>';
    return '<span class="badge badge-red">NE</span>';
}
function getTsaBadge(f) {
    if (f.tsa === 'TSA') return '<span class="badge badge-green">TSA</span>';
    if (f.tsa === 'PARTIAL') return '<span class="badge badge-yellow">MIX</span>';
    if (f.tsa === 'LOCAL') return '<span class="badge badge-yellow">LOK</span>';
    return '<span class="badge badge-gray">—</span>';
}
function getTsaBadgeForSig(tsa) {
    if (tsa === 'TSA') return '<span class="badge badge-green">TSA</span>';
    if (tsa === 'LOCAL') return '<span class="badge badge-yellow">LOK</span>';
    return '<span class="badge badge-gray">—</span>';
}
...
function downloadLocalCSV(files, filename) {
    const BOM = '\\uFEFF';
    const header = 'Cesta;Název;PDF/A;Podpis;Jméno;ČKAIT;Čas.razítko\\n';
    const rows = files.map(f => f.path + ';' + f.name + ';' + (f.pdfaVersion ? 'A-' + f.pdfaVersion : 'NE') + ';' + f.sig + ';' + f.signer + ';' + f.ckait + ';' + f.tsa).join('\\n');
...
def check_pdfa_version(content):
    """Zjistí verzi PDF/A"""
    try:
        # Patterny pro různé formáty zápisu v XMP
        # pdfaid:part='3' nebo pdfaid:part="3" nebo <pdfaid:part>3</pdfaid:part>
        patterns = [
            (rb"pdfaid:part=['\"]?3", 3),
            (rb'pdfaid:part>3<', 3),
            (rb"pdfaid:part=['\"]?2", 2),
    ...
    for i, br in enumerate(byteranges):
        ...
        if name_match:
            ...
            if raw_name.startswith(b'\xfe\xff'):
                sig_info['signer'] = raw_name[2:].decode('utf-16-be', errors='ignore')
            elif b'\x00' in raw_name[:10]:
                sig_info['signer'] = raw_name.decode('utf-16-be', errors='ignore')
            else:
                for enc in ['utf-8', 'windows-1250', 'latin-1']:
                    try:
                        sig_info['signer'] = raw_name.decode(enc)
                        break
                    except:
                        continue
            sig_info['signer'] = sig_info['signer'].replace('\n', '').replace('\r', '').strip()
            if sig_info['signer'].lower() == 'default' or len(sig_info['signer']) < 4:
                sig_info['signer'] = '—'
...
def analyze_pdf_file(filepath):
    ...
    try:
        ...
        return {'name': os.path.basename(filepath), 'pdfaVersion': None, 'pdfaStatus': 'FAIL', 'sig': 'FAIL', 'signer': '—', 'ckait': '—', 'tsa': 'NONE', 'error': str(e)}
...
def analyze():
    """Kontrola jednoho PDF – bez přihlášení (Free trial). Nepotřebuje licenci."""
